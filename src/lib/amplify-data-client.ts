/**
 * Shared Amplify Data Client for Webhook Handlers (Server-Side)
 *
 ...
 *
 * This is separate from the client-side amplify-client.ts because:
 * - API routes run server-side (Lambda functions when deployed)
 * - They need ssr: true configuration
 * - They use apiKey auth mode for publicApiKey() authorization
 *
 * Usage:
 *   import { dataClient, calculateTTL, safeStringify } from '@/lib/amplify-data-client';
 *
 * Based on: handbook/webhooks/examples/shared/amplify-data-client.ts
 */
 ...
 *
 * @example
 * const event = await dataClient.models.SlackEvent.get({ id: 'evt_123' });
 * const parsedData = safeParse(event.data?.rawEvent);
 *
 * if (parsedData) {
 ...
 * @param modelName - Name of the model to create (as keyof Schema)
 * @returns Object with successful and failed counts
 *
 * @example
 * const events = [
 *   { eventId: 'evt_1', eventType: 'message', ... },
 *   { eventId: 'evt_2', eventType: 'message', ... },
 * ];
 *
 * const result = await batchCreate(events, 'SlackEvent');
 * console.log(`${result.successful} succeeded, ${result.failed} failed`);
 */
export async function batchCreate<T extends keyof typeof dataClient.models>(
  items: any[],
  modelName: T
